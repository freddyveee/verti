...
- ~~Chromium-Fassung: verliert die Google-Anmeldung den Neustart?~~ **KEIN FEHLER, geklärt am 08.09.2026.** Freddy hat neu gestartet: Kalender ist da, Anmeldung hält. Die vier Starts, die auf `accounts.google.com/ServiceLogin` liefen (09:41 bis 12:42), waren schlicht „noch nicht angemeldet" — bei Todoist dasselbe Bild. Die Anmelde-Cookies werden gelesen (`last_access_utc` 17:00). Auch meine Zwischenvermutung, der Schlüsselbund-Eintrag `Chromium Safe Storage` vom anders signierten Erstbau mache die Cookies unlesbar, ist damit vom Tisch. Festhalten, weil die Stelle teuer zu untersuchen war: **erst nachsehen, ob überhaupt je angemeldet wurde**, bevor man einen Sitzungsverlust vermutet. (08.09.2026)

- **GRUNDLAGE: die Chromium-Fassung ist ein Entwickler-Bau eines Entwicklungsstands — daher Abstürze und Langsamkeit.** Freddy am 11.09.2026: Stackfield extrem träge (Klicks ohne Wirkung, getippte Buchstaben erscheinen verzögert, Markieren von Leuten geht nicht), regelmäßige Abstürze, „bei Electron war das deutlich besser". Gemessen: **alle vier Abstürze seit 09.09. sind `DCheckLogMessage` → `abort()`** — interne Prüfungen, die in einem Produktionsbau gar nicht mitgebaut würden. Unser Bau hat sie eingeschaltet: `dcheck_always_on = true`, dazu `is_official_build = false`, keine profilgesteuerte Optimierung (`chrome_pgo_phase = 0`), kein ThinLTO. Und der Quelltext ist Chromiums Entwicklungszweig vom 01.09.2026 (155), kein stabiles Release (Chrome stable stand bei 152). Zwei Auslöser: (1) eine IndexedDB-Sperrprüfung (`indexed_db::Connection::IsHoldingLocks`), zweimal — genau das Speicherwerk, das Stackfield und WhatsApp dauernd benutzen; (2) **unser eigener Code**, zweimal: `extensions::ComponentLoader::AddVertiSidebar()` fragt mit `base::PathExists` die Platte ab, und zwar im Hauptthread, wo Blockieren verboten ist. Dazu im Betrieb gemessen: 7,1 GB Arbeitsspeicher über 27 Prozesse, ein Renderer im Leerlauf bei 31 % CPU. (11.09.2026)

- **Spotify spielt in der Chromium-Fassung nicht: die Plattform-Signatur (VMP) fehlt.** Nach dem Entitlement-Fix in 1.2.4 lädt der Entschlüssler, und ein echter Widevine-Stream läuft 8 Sekunden durch — Widevine entschlüsselt also wirklich. Spotify selbst bleibt tot: lädt nicht, nach Neuladen flackert es, ein Lied spielt fünf Sekunden an und hängt sich auf (09.09.2026 von Freddy beschrieben). Genau das Verhalten, das `CLAUDE.md` seit der Electron-Zeit für unsignierte Clients beschreibt. Nachgemessen: Chrome bringt `Google Chrome Framework.sig` mit, **Verti hat keine einzige `.sig`-Datei**. Die DRM-Stufen sind dagegen identisch zu Chrome (Robustheit, dauerhafter Zustand, Gerätekennung), es liegt also nicht daran. **Der bisherige Weg funktioniert hier nicht:** castLabs EVS kennt nur Electron-Pakete und lehnt sowohl Verti als auch Chrome selbst ab („No matching executable found"). Damit braucht es eine Entscheidung: Widevine-Lizenz bei Google, eine kommerzielle Abmachung mit castLabs, Spotifys native Mac-App aufrufen, oder Spotify bleibt in der Chromium-Fassung außen vor. **Am 11.09.2026 im Quelltext BEWIESEN, nicht mehr vermutet:** der Schalter `enable_cdm_host_verification` ist in `media/media_options.gni` fest an `is_chrome_branded` gebunden und steht bei uns auf `false`. Ihn einzuschalten hilft nicht — er ruft `third_party/widevine/scripts/signature_generator.py` auf, und dieser Ordner ist in unserem Quelltext LEER; laut `README.chromium` kommt er aus `chrome-internal.googlesource.com`, Googles privatem Speicher, und braucht zusätzlich ein Signier-Zertifikat. Werkzeug und Schlüssel liegen beide nur bei Google. Selbst herstellen ist damit ausgeschlossen. (09.09.2026, bewiesen 11.09.2026)

- **Chromium-Fassung: das Update-Fenster zeigt eine Adressleiste.** Am 08.09.2026 fotografiert: über dem lila Dialog steht `chrome-extension://llkndemojk…bmkegbopmaafiach/update.html`. In der Electron-Fassung war das ein rahmenloses Fenster ohne Adresse. Ursache ist `chrome.windows.create({type:'popup'})` — Chromium zeigt bei Popups immer den Ursprung. Für ein Produkt, das verkauft wird, sieht das unfertig aus. Möglicher Weg: den Dialog nicht als eigenes Fenster öffnen, sondern wie Bibliothek und Einstellungen als Überlagerung IN sidebar.html (die Mechanik dafür gibt es seit dem 08.09.2026). (08.09.2026)

- **Chromium-Fassung: der Update-Dialog bietet Herunterladen an, obwohl der Updater schon getauscht hat.** Seit 08.09.2026 tauscht Vertis eigener Updater die App selbst aus (bewiesen, 1.2.1 → 1.2.2). Der lila Dialog kennt das nicht und bietet weiter die DMG zum Herunterladen an — 244 MB umsonst, wenn die neue Fassung längst auf der Platte liegt. Richtig wäre dann „Verti neu starten". Dafür muss die Erweiterung die INSTALLIERTE Fassung sehen können, und das geht nur über eine Brücke aus dem Browserprozess (die Erweiterung kennt nur die laufende). (08.09.2026)

- **Chromium-Fassung: Admin-Zugang fehlt.** In der Electron-Fassung entscheidet `istAdminRechner()` am Rechnernamen (`/frederic|freddy/i`) oder `VERTI_ADMIN=1`, ob Einstellungen den Admin-Bereich zeigt (Verbesserungen-Panel, Kompatibilitäts-Check). Eine Chromium-Erweiterung kommt an den Rechnernamen nicht heran, deshalb meldet `get-app-info` dort fest `admin: false` — beide Einträge sind für Freddy unerreichbar. Das Admin-Panel selbst liegt öffentlich unter …github.io/verti/admin.html und ist weiterhin direkt aufrufbar; es fehlt nur der bequeme Weg. Braucht eine Entscheidung, woher die Kennung kommen soll (Schalter beim Start, gespeicherte Einstellung, oder der Browserprozess reicht sie durch). (08.09.2026)
