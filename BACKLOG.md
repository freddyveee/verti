...
- **Chromium-Fassung: die Google-Anmeldung überlebt den Neustart nicht.** Anmelden geht, danach landet man wieder auf der Anmeldeseite. Im Verlauf nachgesehen: bei JEDEM Start läuft `calendar.google.com` → `/calendar/u/0/r` → `accounts.google.com/ServiceLogin`. Die Cookies liegen vollständig und verschlüsselt im Profil (SID, HSID, SSID, APISID, SAPISID, __Secure-1PSID/3PSID, LSID, alle dauerhaft mit echtem Ablauf) — Google sieht sie also nicht. Stärkster Verdacht: der Schlüssel. Der Schlüsselbund-Eintrag heißt `Chromium Safe Storage` (nicht `Verti`), angelegt am 02.09.2026 vom allerersten, noch anders signierten Bau; seither hat jeder neue Bau eine andere Signatur, und Freddy bekam am 08.09. mehrfach die Passwortabfrage. Kommt die App nicht an den Schlüssel, sind die gespeicherten Cookies unlesbar. **Noch nicht bewiesen** — die Messung fehlt. Das ist der wichtigste Punkt von allen: ohne bleibende Anmeldungen ist Verti kein Verti. (08.09.2026)
