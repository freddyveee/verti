...
- **Chromium-Fassung: Spotify spielt nicht, „Wiedergabe von geschützten Inhalten ist nicht aktiviert".** Der Entschlüssler ist da (WidevineCdm 4.10.3050.0, 19 MB, `libwidevinecdm.dylib` im Profil) — er lässt sich nur nicht laden. Am 08.09.2026 nachgemessen: **kein einziger Helfer-Prozess trägt `com.apple.security.cs.disable-library-validation`**, und einen „Plugin"-Helfer, in dem Chrome den Entschlüssler laufen lässt, gibt es in unserem Bau gar nicht (das offene Chromium bringt nur `helper-gpu-entitlements.plist` und `helper-renderer-entitlements.plist` mit, die Plugin-Fassung ist Chrome-intern). Mit gehärteter Laufzeit verbietet macOS damit das Nachladen einer fremd signierten Bibliothek. **Dieselbe Ursache wie in der Electron-Zeit** — dort steht der Schalter längst in `build/entitlements.mac.plist`, siehe CLAUDE.md. (08.09.2026)

- **Chromium-Fassung: verliert die Google-Anmeldung den Neustart? UNGEKLÄRT.** Freddy berichtet: anmelden geht, danach landet man wieder auf der Startseite. Im Verlauf bestätigt für vier Starts hintereinander (09:41, 10:14, 11:47, 12:42 am 08.09.2026): jedes Mal `calendar.google.com` → `/calendar/u/0/r` → `accounts.google.com/ServiceLogin` → `workspace.google.com/products/calendar` — genau die Seite aus dem Screenshot. Todoist lief in denselben Starts ebenfalls auf seine Anmeldeseite. **Aber:** um 16:46 hat er sich neu angemeldet, und um 17:00 wurden die Anmelde-Cookies nachweislich noch GELESEN (`last_access_utc`). Die Sitzung lebt also gerade. Damit ist die naheliegende Erklärung — der Schlüsselbund-Eintrag heißt `Chromium Safe Storage` und stammt vom anders signierten Erstbau, die Cookies wären unlesbar — **widerlegt**. Offen bleibt, ob die vier Starts nur „war noch nicht angemeldet" waren oder ob die Sitzung beim Neustart wirklich verlorengeht. Die Messung ist billig: jetzt neu starten und nachsehen, ob der Kalender noch da ist. Bis dahin nichts bauen. (08.09.2026)

- **Chromium-Fassung: der Verti Browser öffnet eine Fehlerseite.** „Die Website ist nicht erreichbar, verti.browser, DNS_PROBE_FINISHED_NXDOMAIN". Die App trägt im Katalog die Platzhalter-Adresse `https://verti.browser/`; in der Electron-Fassung fing `main.js` sie ab und zeigte `browser.html`. In der Chromium-Fassung fängt sie niemand ab, also fragt Chromium wirklich das DNS. Chromium bringt Tabs und Adressleiste selbst mit — zu klären ist, was der Knopf stattdessen tun soll (neuer Tab? eigene Startseite?). (08.09.2026)

- **Chromium-Fassung: Meldungen kommen nicht an, der Ton erst beim Reinklicken.** Die Ungelesen-Zahl erscheint (die kommt aus dem Seitentitel und geht), aber es klingelt erst, wenn man die App öffnet. Im Profil nachgesehen: unter den Melde-Berechtigungen stehen nur zwei Einträge, **WhatsApp ist nicht dabei**. Ohne Berechtigung gibt es keine Meldung; was man beim Reinklicken hört, ist WhatsApps eigener Ton, den die Seite im Vordergrund nachholt. In Electron meldete die Laufzeit die Berechtigung von sich aus als erteilt (steht in CLAUDE.md), Chromium fragt — und in einem angehefteten Hintergrund-Tab sieht die Frage niemand. (08.09.2026)
