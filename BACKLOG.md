...
- **Spotify spielt in der Chromium-Fassung nicht: die Plattform-Signatur (VMP) fehlt.** Nach dem Entitlement-Fix in 1.2.4 lädt der Entschlüssler, und ein echter Widevine-Stream läuft 8 Sekunden durch — Widevine entschlüsselt also wirklich. Spotify selbst bleibt tot: lädt nicht, nach Neuladen flackert es, ein Lied spielt fünf Sekunden an und hängt sich auf (09.09.2026 von Freddy beschrieben). Genau das Verhalten, das `CLAUDE.md` seit der Electron-Zeit für unsignierte Clients beschreibt. Nachgemessen: Chrome bringt `Google Chrome Framework.sig` mit, **Verti hat keine einzige `.sig`-Datei**. Die DRM-Stufen sind dagegen identisch zu Chrome (Robustheit, dauerhafter Zustand, Gerätekennung), es liegt also nicht daran. **Der bisherige Weg funktioniert hier nicht:** castLabs EVS kennt nur Electron-Pakete und lehnt sowohl Verti als auch Chrome selbst ab („No matching executable found"). Damit braucht es eine Entscheidung: Widevine-Lizenz bei Google, eine kommerzielle Abmachung mit castLabs, oder Spotify bleibt in der Chromium-Fassung außen vor. Starker Hinweis, kein Beweis — der wäre nur mit einem signierten Bau zu führen. (09.09.2026)
