...
- **Chromium-Fassung: die Update-MELDUNG sieht Vorab-Fassungen nicht.** `sw.js` fragt `/releases/latest`, und GitHub lässt dort Vorab-Fassungen weg — genau die Falle, die der Update-Server schon einmal hatte (behoben am 08.09.2026). Gemessen am 08.09.2026 nach der Veröffentlichung von 1.2.2: `/releases/latest` liefert v1.1.18 (Electron), verglichen mit der installierten 1.2.1 ist das älter, also erscheint kein lila Update-Hinweis. Der Omaha-Updater tauscht die App trotzdem aus, aber still — und „nichts still im Hintergrund" ist Freddys ausdrückliche Regel. Behebung wie beim Server: die Liste holen (`/releases?per_page=15`) und das erste Release nehmen, das ein CRX3-Paket mitbringt; dann beschreibt die Meldung genau die Fassung, die der Updater auch installiert. Braucht einen neuen Bau und ein neues Vorab-Release. (08.09.2026)
