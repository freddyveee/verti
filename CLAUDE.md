...
- Electron ist seit 1.0.21 die castLabs-Variante „Electron for Content Security" (ECS, `github:castlabs/electron-releases#v43.2.0+wvcus`), damit Spotify (Widevine-DRM) läuft. Folgen: (1) `main.js` wartet beim Start auf `components.whenReady()` (Widevine-CDM, ~10 MB einmalig pro Profil); (2) `build/entitlements.mac.plist` braucht `com.apple.security.cs.disable-library-validation`, sonst lädt der CDM in der gepackten App nicht (dlopen-Fehler, Spotify schwarz/stumm); (3) `scripts/ecs-afterpack.js` entfernt beim Universal-Build die mitgelieferten `.sig`-Dateien der x64/arm64-Zwischenbauten und holt danach die VMP-Produktionssignatur bei castLabs EVS, auf dem Mac VOR dem Codesign; unter Windows macht das `scripts/ecs-aftersign.js` im `afterSign`-Hook, weil electron-builder die Exe nach `afterPack` noch bearbeitet (Icon/Metadaten) und das jede frühere VMP-Signatur bricht (im Build-Log muss „Signature is valid: streaming" NACH „signing with signtool.exe path=dist/win-unpacked/Verti.exe" stehen); (4) `electronDownload.mirror` zeigt auf die castLabs-Releases (ohne „v" am Ende). Entwicklungs-Signatur der castLabs-Builds reicht für Spotify NICHT, nur EVS-signierte Builds spielen. castLabs hinkt Electron ein paar Patch-Versionen hinterher; Updates über `npm install "https://github.com/castlabs/electron-releases#vX.Y.Z+wvcus" --save-dev` (Tags unter github.com/castlabs/electron-releases/releases). Electron-Fuses: castLabs signiert seit ECS v35 GENAU EINEN festen Satz mit, Abweichungen lehnt EVS ab. Dieser Satz ist in `scripts/ecs-afterpack.js` als `ECS_FUSES` hinterlegt und wird UNMITTELBAR VOR `vmpSign` umgelegt (Mac im afterPack, Windows im afterSign) - jede spätere Änderung an der Binärdatei macht die VMP-Signatur ungültig. **Niemals `build.electronFuses` von electron-builder benutzen**, das legt sie erst nach `afterPack` um und zerstört die Signatur still (Spotify wäre tot). Am 02.09.2026 gemessen: mit gesetzten Fuses bleibt „Signature is valid: streaming" erhalten, Notarisierung läuft durch. Wichtigster Schalter ist `EnableCookieEncryption` - ohne ihn speichert Chromium die Anmelde-Cookies im KLARTEXT (gemessen: 556 von 557 unverschlüsselt, während Chrome 2742 von 2742 verschlüsselt). **ACHTUNG, kein Zurück:** Öffnet eine Version OHNE diesen Schalter ein verschlüsseltes Profil, ist der Cookie-Bestand danach leer - ein zurückgerolltes Release loggt alle aus. Und `npx electron .` teilt sich das Profil mit der installierten App, deshalb im Dev ab jetzt IMMER `VERTI_USER_DATA=…` setzen.
...
- **Vertis CRX3-Signierschluessel (ab dem Chromium-Umbau, 02.09.2026):** liegt NUR in `~/Verti-Signing/verti-crx3.der` und gehoert wie das Mac-Zertifikat gesichert. Damit werden die Update-Pakete signiert; sein Fingerabdruck steht als `crx_pkhash` im Chromium-Patch, und Vertis Updater nimmt **ausschliesslich** Pakete an, die damit signiert sind. Geht der Schluessel verloren, muss ein neuer erzeugt, der Fingerabdruck im Patch geaendert und der Updater neu gebaut werden - erst danach koennen Nutzer wieder Updates bekommen. Ein neuer Schluessel allein reicht NICHT
...
- Stackfield-Badge (ab 1.0.20, Code von Stackfield am 22.08.2026 aus dem Verti-Cache gelesen): Stackfield schreibt nichts in den Titel, zeigt die Ungelesen-Zahl aber per Favico.js im Favicon (`ShowPageTitle()` → `favicon.badge(n)`/`reset()`). `view-preload.js` fängt den `Favico`-Konstruktor ab und meldet die Zahl als `pageCounts` an main.js (exakt, sinkt beim Lesen, wird beim Öffnen nicht genullt). WICHTIG: nur echte Zahlen ergeben ein Badge; ein nicht-numerischer Favico-Wert (reiner Punkt-Indikator, den Stackfield auch ohne Ungelesenes setzt) wird als 0 gewertet, sonst Phantom-1 (25.08.2026 bei Cindy beobachtet). Native Stackfield-Meldungen kommen per `new Notification()` – aber nur, wenn im Stackfield-Profil „Desktop-Benachrichtigungen" an sind (Server-Einstellung; Stackfields Aktivierungs-Banner erscheint in Verti nie, weil Electron die Berechtigung von sich aus als „granted" meldet)
