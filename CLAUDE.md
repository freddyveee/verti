...
- Maus-Seitentasten (ab 1.0.20): Chromium navigiert bei „back"/„forward"-Maustasten nur, wenn die Seite das mouseUp nicht verbraucht. Verti fängt die Tasten per `before-mouse-event` + `preventDefault` VOR der Seite ab (`attachMouseNav`) und navigiert selbst; Windows zusätzlich `app-command`, ein 250-ms-Riegel (`mouseNav`) verhindert Doppelnavigation. Test von hier aus nicht möglich (CGEvent-Posting ohne Bedienungshilfen-Freigabe), nur mit echter Maus
